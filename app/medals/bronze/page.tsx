...
export default function BronzeMedalsPage() {
  const [selectedRecord, setSelectedRecord] = useState<MedalRecord | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Pre-filter to bronze medals only
  const bronzeMedals = medals.filter((m) => m.medal === "bronze");
    ...
    <div className="container-wide py-12">
      ...
      {bronzeMedals.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-sm text-navy-400">No bronze medals found</p>
        </div>
      ) : (
      ...
      <DetailModal
        ...
        title="Olympic Bronze Medal Detail"
      >
        {selectedRecord ? (
